...
import { toSession, toSessionDocument } from '../mappers/mongo.session.mapper.js';

export class MongoSessionRepository implements SessionRepository {
    constructor(private sessionCollection: Collection<SessionDocument>) {}

    create = async (session: Session): Promise<void> => {
        await this.sessionCollection.insertOne(toSessionDocument(session));
    };
